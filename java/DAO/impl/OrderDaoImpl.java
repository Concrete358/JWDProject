...
public class OrderDaoImpl implements OrderDAO {
	...
	public Optional<Order> add(Order order) throws DaoException {
		...
		try (PreparedStatement psCheck = connection.prepareStatement(SQL_ORDER_RENT_CHECK);
			 PreparedStatement psAdd = connection.prepareStatement(SQL_ORDER_CREATE)) 
	...
	public void closeOrder(int orderId) throws DaoException {
		Connection connection = ConnectionPool.getInstance().getConnection();
		try(PreparedStatement ps = connection.prepareStatement(SQL_ORDER_CLOSE)) {
			ps.setInt(1, orderId);
			ps.executeUpdate();
		} catch(SQLException e) {
	...
	public List<Order> findAll(int limit, int pageNumber) throws DaoException {
		...
		try {
			...
			ps = conn.prepareStatement(SQL_ORDER_LIST_SELECT);
			ps.setInt(1, limit);
	...
	public List<Order> findAllByUserId(int userId, int limit, int pageNumber) throws DaoException {
		...
		try {
			conn = ConnectionPool.getInstance().getConnection();
			ps = conn.prepareStatement(SQL_ORDER_FIND_ALL_BY_USER_ID);
			ps.setInt(1, userId);			
			ps.setInt(2, limit);
			int offset = limit * pageNumber - limit;
			ps.setInt(3, offset);
	...
	public List<Order> findActiveByCarId(int carId) throws DaoException {
		...
		try {
			conn = ConnectionPool.getInstance().getConnection();
			ps = conn.prepareStatement(SQL_ORDER_FIND_ACTIVE_BY_CAR_ID);
			ps.setInt(1, carId);			
			ResultSet rs = ps.executeQuery();
			List<Order> listOfOrders = new ArrayList<Order>();
			while (rs.next()) {
	...
	public Optional<Order> findById(int orderId) throws DaoException {
		...
		try {
			conn = ConnectionPool.getInstance().getConnection();
			ps = conn.prepareStatement(SQL_ORDER_FIND_BY_ID);
			ps.setInt(1, orderId);			
			ResultSet rs = ps.executeQuery();
			if(!rs.next()) {
				return Optional.empty();
	...
	public void setOrderStatusActive(int orderId) throws DaoException {
		Connection connection = ConnectionPool.getInstance().getConnection();
		try(PreparedStatement ps = connection.prepareStatement(SQL_ORDER_SET_ORDER_ACTIVE)) {
			ps.setInt(1, orderId);
			ps.executeUpdate();
		} catch(SQLException e) {
	...
	public int countAll() throws DaoException {
		...
		try {
			conn = ConnectionPool.getInstance().getConnection();
			ps = conn.prepareStatement(SQL_ORDER_COUNT_ALL);
			ResultSet rs = ps.executeQuery();
			rs.next();
	...
	public int countAllByUserId(int userId) throws DaoException {
		...
		try {
			conn = ConnectionPool.getInstance().getConnection();
			ps = conn.prepareStatement(SQL_ORDER_COUNT_ALL_BY_USER_ID);
			ps.setInt(1, userId);
			ResultSet rs = ps.executeQuery();
			rs.next();
			return rs.getInt("count(order_id)");
