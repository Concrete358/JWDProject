...
public class RepairOrderDaoImpl implements RepairOrderDAO {
	...
	public List<RepairOrder> findAll(int limit, int pageNumber) throws DaoException {
		try(ConnectionManager cm = new ConnectionManager();
				PreparedStatement ps = cm.getConnection().prepareStatement(SQL_REPAIR_ORDER_LIST_SELECT)) 
		{
			ps.setInt(1, limit);
	...
	public int countAll() throws DaoException {
		try(ConnectionManager cm = new ConnectionManager();
				PreparedStatement ps = cm.getConnection().prepareStatement(SQL_REPAIR_ORDER_COUNT_ALL)) 
		{
			ResultSet rs = ps.executeQuery();
			rs.next();
	...
	public int add(int orderId, int repairCost, String damageDescription) throws DaoException {
		try(ConnectionManager cm = new ConnectionManager();
			PreparedStatement psAdd = cm.getConnection().prepareStatement(SQL_REPAIR_ORDER_ADD);
			PreparedStatement psGet = cm.getConnection().prepareStatement(SQL_REPAIR_ORDER_FIND_BY_ORDER_ID))
		{
	...
	public Optional<RepairOrder> findByRepairOrderId (int repairOrderId) throws DaoException {
		try(ConnectionManager cm = new ConnectionManager();
			PreparedStatement ps = cm.getConnection().prepareStatement(SQL_REPAIR_ORDER_FIND_BY_REPAIR_ORDER_ID)) 
		{
	...
	public void update(int repairCost, String damageDescription, int repairOrderId) throws DaoException {
		try(ConnectionManager cm = new ConnectionManager();
			PreparedStatement ps = cm.getConnection().prepareStatement(SQL_REPAIR_ORDER_UPDATE)){
